...
date = "2016-01-26"
district_id = 2
x = formatplot("demand %s (%s) - district %s" % (date, weekday(date), district_id), ymax=1000)
#d, s, g = getdsg("results", district_id, date, db)
d, s, g = getdsg("diditest.gaps", district_id, date, db)
plt.plot(x, d, '-', label="test", color="red")
# plt.plot(x, s, '-*', label="supply", color="orange")
# plt.plot(x, g, '-+', label="22", color="black")
d, s, g = getdsg("results_test", district_id, date, db)
plt.plot(x, d, '+', label="results_test", color="green")
...
date = "2016-01-19"
d, s, g = getdsg("gaps", district_id, date, db)
plt.plot(x, d, '-', label=date, color="cyan")
