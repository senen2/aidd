...
def estimate3(table_source, date, slot, case, days_before, fungap, case_gap):
    gap = fungap(table_source, date_est(date, days_before), slot)
    # gap = read_gap(table_source, date_est(date, days_before), slot)
    # gap = read_gap_prom(table_source, date_est(date, days_before), slot)
    for i in xrange(1,len(case)+2):
...
def update3(table_results, table_source, days_before, case, fungap, case_gap): # cambio
    for date in table_results:
        for slot in table_results[date]:
            table_results[date][slot] = estimate3(table_source, date, slot, case, days_before, fungap, case_gap)
...
def what_cases(district_id, table_results, table_source, table_test, days_before, fungap, epo):
    case = read_case(district_id)
    case_gap = read_case_gap(district_id)
    # if case_gap:
    #     pass
    # else:
    #     case_gap = [1]
    if case:
        ...
        l = read_l_2(district_id)
        for itera in xrange(0, epo): #255
            update3(table_results, table_source, days_before, case, fungap, case_gap)
            score = testdist(table_results, table_test)
            if score <= best_score:
                best_score = score
                save_score(district_id, best_score)
                best_case_gap = list(case_gap)
                save_case_gap(district_id, best_case_gap)
                case_gap[len(case_gap)-1] += 1
                l = 0
                save_l_2(district_id, l)
            elif l == 1:
                case_gap[len(case_gap)-1] += 1
            else:
                case_gap[len(case_gap)-1] -= 1
                case_gap.append(case_gap[len(case_gap)-1]+1)
                l = 1
                save_l_2(district_id, l)
            print best_case_gap
            print itera+1, best_score, score, "cuantos if:", len(best_case_gap)
    else:
        case = [1]
        best_score = read_score(district_id)
        l = read_l_2(district_id)
...
def list_cases2(district_id, table_results, table_source, table_test, days_before, fungap, epo):
    case = read_case(district_id)
    if case:
        best_score = read_score(district_id)
        best_case = list(case)
        ...
        for itera in xrange(0, epo): #255
            ...
            if score <= best_score:
                ...
                best_case_gap = []
                for ii in xrange(1,len(best_case)+2):
                    best_case_gap.append(ii)
                save_case_gap(district_id, best_case_gap)
                case[len(case)-1] += 1
                l = 0
...
def train_district_multi(multi):
    ...
    for n in xrange(1, 11):
        list_cases(district_id, table_results, table_source, table_test, days_before, fungap, (50*n)*n)
        clean_case(district_id, table_results, table_source, table_test, days_before, fungap)
...
#             train_district(district["district_id"], 7, globals()['read_gap'], "results_test_roma_3", "prueba5")
    ...
    for district in districts:
        for scene in scenes:
            multi.append([int(district["district_id"]), int(scene["days_before"]), globals()[scene["fungap"]], scene["table_test"], int(scene["id"])])

    # pool = Pool(processes=6)
    # pool.map(train_district_multi, multi)
    # pool.close()
    # pool.join()
    
    for m in multi:
        if m[0] == 51:
            train_district_multi2(m)
